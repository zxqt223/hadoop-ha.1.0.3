...
//  private InetSocketAddress selfAddr;
  ...
  private OfferService primaryService=null;

  static{
	  Configuration.addDefaultResource("ha-site.xml");
  }
  
  public AvatarDataNode(Configuration conf, AbstractList<File> dataDirs,SecureResources resources) 
    throws IOException {
  ...
  void startDataNode(Configuration conf, 
    ...
    /**
     * @ADDCODE xianquan.zhang 2013-5-2
     * add this code to support webhdfs,because the old code does not set attribute for configuration,then the browseDirectory.jps 
     * throw exception for Nullpoint.
     *  
  ...
  private synchronized NamespaceInfo handshake(boolean startup) throws IOException {
    ...
    do {
    	/**@ADDCODE xianquan.zhang connect master namenode*/
      if(startup){
	    try {
	      String primaryAddress =getPrimaryAddress();
	      String firstNNAddress = "hdfs://"+nameAddr1.getHostName() + ":" +
	        nameAddr1.getPort();
      ...
      try {
    	if ((firstIsPrimary && startup) || !startup) {
		  if (namenode1 == null) {
		    namenode1 = (DatanodeProtocol) RPC.getProxy(DatanodeProtocol.class,DatanodeProtocol.versionID,nameAddr1,getConf());
	        ((DatanodeProtocols)namenode).setDatanodeProtocol(namenode1, 0);
	      }
		  if (avatarnode1 == null) {
	        avatarnode1 = (AvatarProtocol) RPC.getProxy(AvatarProtocol.class,AvatarProtocol.versionID,avatarAddr1,getConf());
	      }
		  if (startup) {
        ...
        LOG.info("Server at " + nameAddr1 + " not available yet, try it again ......");
      ...
      try {
        ...
        LOG.info("Server at " + nameAddr2 + " not available yet, try it again ......");
      } catch(SocketTimeoutException te) {  // namenode is busy
        LOG.info("Problem connecting to server timeout. " + nameAddr2);
      }
    } while (startup && nsInfo == null);
    return nsInfo;
  }
 ...
 /**
  ...
  public static AvatarDataNode makeInstance(String[] dataDirs, Configuration conf,SecureResources resources)
    throws IOException {
    ...
    if (dirs.size() > 0)
      return new AvatarDataNode(conf, dirs,resources);
  ...
  public static AvatarDataNode createDataNode(String args[],
                                 Configuration conf,SecureResources resources) throws IOException {
    AvatarDataNode dn = instantiateDataNode(args, conf,resources);
    runDatanodeDaemon(dn);
    return dn;
  }
 /**@ADDCODE xianquan.zhang */
  public synchronized boolean isPrimaryService(OfferService service) {
    boolean res;
    if(this.primaryService==null){
      ...
      if(primaryAddress.contains(service.avatarnodeAddress.getHostName())){
      ...
      res=this.primaryService==service;
    }
    return res;
  }
  
  private String getPrimaryAddress() throws IOException, KeeperException,InterruptedException{
    ZooKeeperClient zkClient=ZooKeeperClient.getInstance(new Configuration());
	String primaryAvatarAddress= zkClient.getPrimaryAvatarAddress(true);
	try {
	  zkClient.shutdown();
	} catch (InterruptedException e) {
	  LOG.warn("Fail to close zookeeper client!",e);
	}
	return primaryAvatarAddress;
  }
  public synchronized void clearPrimaryService(OfferService service){
    if(this.primaryService==service){
  ...
  public static void main(String argv[]) {
    try {
      ...
      AvatarDataNode avatarnode = createDataNode(argv, null,null);
      if (avatarnode != null)
        avatarnode.join();
    } catch (Throwable e) {
