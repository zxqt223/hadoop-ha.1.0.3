...
public class RPC {
  ...
  /**@ADDCODE xianquan.zhang 2013-5-3 , namenode HA switch invoker*/
  private static class RPCRetryForNamenodeHASwitchInvoker extends HadoopInvoker{
    ...
    public RPCRetryForNamenodeHASwitchInvoker(Class<? extends VersionedProtocol> protocol,
        InetSocketAddress address, UserGroupInformation ticket,
        Configuration conf, SocketFactory factory,
        int rpcTimeout) throws IOException {
      this.conf=conf;
    ...
    public Object invoke(Object proxy, Method method, Object[] args)
      ...
      try{
    	...
    	long timeout=conf.getLong("ha.zookeeper.timeout", 30)*1000;
    	LOG.warn("Fail to call namenode : "+remoteId.getAddress().toString()+",may namenode happen failover and trg again for sleep "+timeout+"ms",e);
    	Thread.sleep(timeout);
    	InetSocketAddress address=getPrimaryNamenodeAddr(this.conf);
    	this.remoteId = Client.ConnectionId.getConnectionId(address, protocol,
    	          ticket, rpcTimeout, conf);
    	value=(ObjectWritable)
        client.call(new Invocation(method, args), remoteId);
  ...
  /**@ADDCODE xianquan.zhang 2012-8-17*/
  ...
  /**@ADDCODE xianquan.zhang 2013-5-3 namenode ha*/
  public static VersionedProtocol getNameNodeHAProxy(
      Class<? extends VersionedProtocol> protocol,
    ...
    if(conf.get("fs.default.name0", "").contains(strAddr)||conf.get("fs.default.name1", "").contains(strAddr)){
    	proxy =(VersionedProtocol) Proxy.newProxyInstance(
	            protocol.getClassLoader(), new Class[] { protocol },
	            new RPCRetryForNamenodeHASwitchInvoker(protocol, getPrimaryNamenodeAddr(conf), ticket, conf, factory, 0));
    }else{
    	proxy =(VersionedProtocol) Proxy.newProxyInstance(
	            protocol.getClassLoader(), new Class[] { protocol },
	            new Invoker(protocol, addr, ticket, conf, factory, 0));
    }
  ...
  private static InetSocketAddress getPrimaryNamenodeAddr(Configuration conf) throws IOException{
	ZooKeeperClient zkClient=ZooKeeperClient.newInstance(conf);
  	InetSocketAddress address =null;
  	String primaryNNAddress=null;
	try {
	  primaryNNAddress = zkClient.getPrimaryAvatarAddress(true);
	} catch (KeeperException e) {
	  throw new IOException("Fail to get primary Namenode address!",e);
	} catch (InterruptedException e) {
	  throw new IOException("Fail to get primary Namenode address!",e);
	}
  	address = NetUtils.createSocketAddr(((URI.create(primaryNNAddress)).getAuthority()),8020);
  	try {
	  zkClient.shutdown();
	} catch (InterruptedException e) {
	  LOG.warn("Fail to shutdown zookeeper client.",e);
	}
  	return address ;
  }
  /** Construct a client-side proxy object that implements the named protocol,
