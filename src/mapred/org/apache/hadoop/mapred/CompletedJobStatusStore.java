...
class CompletedJobStatusStore implements Runnable {
  ...
  /**@CHANGECODE xianquan.zhang  Reconnect fs use this var**/
  private Configuration conf=null;
  /***@ADDCODE xianquan.zhang */
  private void reconnectFS() throws IOException{
  ...
  private void deleteJobStatusDirs() {
       ...
       */
      /**@ADDCODE xianquan.zhang */
     FileStatus[] jobInfoFiles=null;
    
     try{
    	 jobInfoFiles = fs.listStatus(
                 new Path[]{new Path(jobInfoDir)});
    	...
    	try {
			...
			jobInfoFiles = fs.listStatus(
			 ...
			 LOG.warn("Could not obtain job info files : " + ie.getMessage(), e);
			return ;
		}
     }
  ...
  private FSDataInputStream getJobInfoFile(JobID jobId) throws IOException {
    ...
    boolean res;
    try{
    	res=fs.exists(jobStatusFile);
    }catch(IOException e){
		reconnectFS();
		res=fs.exists(jobStatusFile);
    }
    if(res){
    	return fs.open(jobStatusFile); 
    }
    return null;
    /**
     *old code
