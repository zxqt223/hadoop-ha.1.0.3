...
/**
  ...
  private Path getFinalPath(Path jobOutputDir, Path taskOutput, 
    ...
    if (taskOutputUri == relativePath) {//taskOutputPath is not a parent of taskOutput
      /**xianquan.zhang(2012-11-16) happen failover whether or not**/
      FileSystem fs=taskOutput.getFileSystem(new Configuration());
	  taskOutput=fs.validatePath(taskOutput);
	  taskOutputPath=fs.validatePath(taskOutputPath);
	  jobOutputDir=fs.validatePath(jobOutputDir);
	  relativePath = taskOutputPath.toUri().relativize(taskOutputUri);
